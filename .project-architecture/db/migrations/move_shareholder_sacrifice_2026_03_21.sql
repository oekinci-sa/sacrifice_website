-- rpc_move_shareholder_to_sacrifice: hissedarı başka kurbanlığa taşır (tek transaction).
-- Kaynak kurbanlıkta empty_share +1, hedefte -1; toplam/kalan tutar yeni hisse bedeline göre.
-- Kurbanlık satırları LEAST/GREATEST(sacrifice_id) sırasıyla FOR UPDATE (deadlock önlemi).
...
BEGIN
  ...
  ELSIF (TG_OP = 'UPDATE') THEN
    ...
    IF NEW.sacrifice_id IS DISTINCT FROM OLD.sacrifice_id THEN
