...
BEGIN
  ...
  FOR r IN
    ...
    SET
      ...
      delivered_share_kg = CASE
        WHEN NOT (p_patch ? 'delivered_share_kg') THEN sa.delivered_share_kg
        WHEN p_patch->'delivered_share_kg' IS NULL OR jsonb_typeof(p_patch->'delivered_share_kg') = 'null' THEN NULL
        ELSE (p_patch->>'delivered_share_kg')::numeric
      END,
      delivery_notes = CASE
        WHEN NOT (p_patch ? 'delivery_notes') THEN sa.delivery_notes
        WHEN p_patch->'delivery_notes' IS NULL OR jsonb_typeof(p_patch->'delivery_notes') = 'null' THEN NULL
        ELSE NULLIF(trim(p_patch->>'delivery_notes'), '')
      END,
      foundation = CASE
      ...
      planned_delivery_time = CASE
        ...
        WHEN p_patch ? 'sacrifice_time' THEN ((p_patch->>'sacrifice_time')::time + (v_offset || ' minutes')::interval)::time
