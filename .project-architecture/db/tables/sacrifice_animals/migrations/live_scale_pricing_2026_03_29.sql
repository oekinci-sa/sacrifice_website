-- Canlı baskül: pricing_mode, toplam kg/tutar; share_price canlı modda NULL.
-- rebalance_live_scale_shareholders + RPC güncellemeleri

ALTER TABLE public.sacrifice_animals
  ADD COLUMN IF NOT EXISTS pricing_mode text NOT NULL DEFAULT 'fixed'
    CHECK (pricing_mode IN ('fixed', 'live_scale'));

ALTER TABLE public.sacrifice_animals
  ADD COLUMN IF NOT EXISTS live_scale_total_kg numeric;

ALTER TABLE public.sacrifice_animals
  ADD COLUMN IF NOT EXISTS live_scale_total_price numeric(12,2);

ALTER TABLE public.sacrifice_animals
  ALTER COLUMN share_price DROP NOT NULL;
...
ALTER TABLE public.sacrifice_animals
  ADD CONSTRAINT sacrifice_animals_pricing_consistency CHECK (
    (pricing_mode = 'fixed' AND share_price IS NOT NULL)
    OR (pricing_mode = 'live_scale' AND share_price IS NULL)
  );

-- Kişi başı: live_scale_total_price kuruş olarak n hissedar arasında bölünür (kalan ilk sıralara)
