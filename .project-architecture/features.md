...
- **Faz 3 (sacrifice_animals):** `rpc_create_sacrifice`, `rpc_update_sacrifice_core` (çekirdek alanlar + `empty_share` + kesim/teslim zamanları), `rpc_delete_sacrifice` — `app.actor` + `log_sacrifice_changes`. `rpc_update_sacrifice_timing` / `rpc_update_sacrifice_share` kaldırıldı; aynı iş `rpc_update_sacrifice_core` patch ile yapılır (bkz. [audit-rpc-and-triggers.md](audit-rpc-and-triggers.md)). `POST /api/create-sacrifice`, `PUT /api/update-sacrifice`, `POST /api/update-sacrifice-timing`, `POST /api/update-sacrifice-share`, `POST /api/reset-shares` bu RPC’leri kullanır; kurban güncellemede `last_edited_by_display` döner. **Silme:** `DELETE /api/sacrifices/[id]` → `rpc_delete_sacrifice` (aynı transaction’da hissedar + rezervasyon + kurban; `change_logs` silende `app.actor`).
- **Canlı baskül (`pricing_mode: live_scale`):** Toplam kg/tutar kolonları; canlıda `share_price` null. Hisse Al tablosunda «Hisse Bedeli» yalnız **Canlı Baskül** (kg/tutar public satırda yok; hisse sorgula / hissedar detayında gösterim). Admin kurbanlıklarda «Hisse Bedeli» sütunu genişletildi; canlı satırlarda alt satırda kg · TL. `share_price` → null geçişinde hissedar senkronu: [changelogs/changelog-2026-03-live-scale-pricing.md](changelogs/changelog-2026-03-live-scale-pricing.md) ve migration `fix_live_scale_shareholder_sync_trigger_2026_03_30.sql`.
