...
export const SuccessView = ({ onPdfDownload }: SuccessViewProps) => {
  ...
  const { formData, selectedSacrifice } = useHisseStore();
  ...
  const formatPhoneNumber = (phone: string): string =>
    phone.replace(/\D/g, "").replace(/^90(\d{3})(\d{3})(\d{2})(\d{2})$/, "0$1 $2 $3 $4");
  ...
  const createReceiptDataFromDb = (shareholder: any) => {
    ...
    const formattedPhoneNumber = formatPhoneNumber(shareholder.phone_number || shareholder.phone || "");
    
    return {
      // Hisse Sahibi Bilgileri
      shareholder_name: shareholder.shareholder_name || shareholder.name || "Müşteri",
      phone_number: formattedPhoneNumber,
      delivery_location: formatDeliveryLocation(shareholder.delivery_location || ""),
  ...
  const downloadPdf = (blob: Blob, shareholderName: string | undefined) => {
    ...
    const safeName = shareholderName ? shareholderName.replace(/\s+/g, '-') : 'hissedar';
  ...
  const renderLoadingSkeletons = () => (
    <div className="mt-8 w-full max-w-xl mx-auto">
      ...
      {[1, 2, 3].map((i) => (
        <div key={i} className="border rounded-lg p-3 flex justify-between items-center bg-gray-50 mb-3">
  ...
  const renderErrorMessage = () => (
    <Alert variant="destructive" className="mt-8 max-w-xl mx-auto">
      ...
      <AlertDescription>
        ...
        {dbError && <p className="mt-2 text-xs">{(dbError as Error).message}</p>}
      </AlertDescription>
    </Alert>
  );
  ...
  const getShareholderDisplayName = (shareholder: any) => {
    return shareholder.shareholder_name || shareholder.name || "İsimsiz Hissedar";
  };
  
  const getShareholderDisplayPhone = (shareholder: any) => {
    return formatPhoneNumber(shareholder.phone_number || shareholder.phone || "");
  };
          ...
          {isClient && (
            <>
              ...
              {dbData && dbData.shareholders && dbData.shareholders.length > 0 && (
                <div className="mt-8">
                  <h2 className="text-lg sm:text-xl font-semibold text-center mb-4">Hissedar Bilgi Dökümanları</h2>
                  <div className="grid gap-3 max-w-xl mx-auto">
                    {dbData.shareholders.map((shareholder: any, index: number) => (
                      <div key={index} className="border rounded-lg p-3 flex justify-between items-center bg-gray-50">
                        <div>
                          <p className="font-medium">{getShareholderDisplayName(shareholder)}</p>
                          <p className="text-sm text-gray-500">{getShareholderDisplayPhone(shareholder)}</p>
                        </div>
                        
                        <BlobProvider document={<ReceiptPDF data={createReceiptDataFromDb(shareholder)} />}>
                          {({ blob, url, loading, error }) => (
                            <Button
                              ...
                              onClick={() => blob && downloadPdf(blob, getShareholderDisplayName(shareholder))}
              ...
              {(!dbData || !dbData.shareholders || dbData.shareholders.length === 0) && 
               !isDbLoading && !isDbError && (
                <Alert variant="destructive" className="mt-8 max-w-xl mx-auto">
                  ...
                  <AlertDescription>
                    Hissedar bilgileri veritabanından alınamadı. Lütfen sistem yöneticisiyle iletişime geçin.
                  </AlertDescription>
                </Alert>
              )}
            </>
